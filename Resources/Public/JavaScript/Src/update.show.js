...
var $buo_show = function() {
    ...
    if (!op.url) op.url = burl + ((op.l && (op.l + "/")) || "") + "update-browser.html" + (op.test ? "?force_outdated=true" : "") + "#" + op.jsv + ":" + op.pageurl;
    ...
    t.sq = {
        ...
        'msgmore': 'Përditësoni shfletuesin tuaj për më tepër siguri, shpejtësi dhe më të mirin e funksionimeve në këtë sajt.',
        'bupdate': 'Përditëso shfletuesin',
        'bignore': 'Shpërfille',
        'remind': 'Do të rikujtoheni pas {days} ditësh.',
    ...
    if (custom_text)
        if (typeof custom_text === 'string') t = ta = custom_text;
        else
            for (var i in custom_text) ta[i] = custom_text[i];
    if (ta.msg) t = '<b class="buorg-mainmsg">' + t.msg + '</b> <span class="buorg-moremsg">' + t.msgmore + '</span> <span class="buorg-buttons"><a{up_but}>' + t.bupdate + '</a> <a{ignore_but}>' + t.bignore + '</a></span>';
    var tar = "";
    ...
    if (ta.msg && (op.ll == "ar" || op.ll == "he" || op.ll == "fa")) style2 += '<style>.buorg {direction:RTL; unicode-bidi:embed;}</style>';
    if (!ta.msg && t.indexOf && t.indexOf("%s") !== -1) {
        ...
        if (op.style === "bottom") style2 += '<style>.buorg {bottom:0; top:auto; border-top:1px solid #A29330; } @keyframes buorgfly {from {opacity:0;transform:translateY(50px)} to {opacity:1;transform:translateY(0px)}}</style>';
        else if (op.style === "corner") style2 += '<style> .buorg { text-align: left; width:300px; top:50px; right:50px; left:auto; border:1px solid #A29330; } .buorg-buttons, .buorg-mainmsg, .buorg-moremsg { display: block; } .buorg-buttons a {margin: 4px 2px;} .buorg-icon {display: none;}</style>';
    ...
    if (updatebutton) updatebutton.onclick = function(e) {
        div.onclick = null;
        op.onclick(op);
    ...
    if (ignorebutton) {
        ...
        if (op.noclose || op.reminderClosed == 0) ignorebutton.parentNode.removeChild(ignorebutton);
    }
    op.onshow(op);
